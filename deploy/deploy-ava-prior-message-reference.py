...
"""Exact-CAS Ava message-deposit guard activation for prior-message references."""
...
TARGET = Path("core/pipeline_message_deposit.py")
OWNER = {"uid": 1001, "gid": 1001}
BEFORE = {"sha256": "fcbb49f2b771c9bcf939b7ab2ccad0c26c7d04462aabad1b2149fdab50041d47",
          "size": 28322, "mode": "0644", **OWNER}
AFTER = {"sha256": "2cb7de41b528e72f3bf054001c790f4e775e941c38c54801c51940402a78732e",
         "size": 33214, "mode": "0644", **OWNER}
...
def atomic_bytes(path, data, metadata, before_replace=None):
    safe_path(path)
    fd, temporary = tempfile.mkstemp(prefix=".ava-prior-message-", dir=path.parent)
    try:
...
class Native:
    ...
    def syntax(self, source):
        self.run(["docker", "exec", "-i", "-e", "PYTHONDONTWRITEBYTECODE=1", "ai_engine",
                  "python", "-B", "-c", "import sys;compile(sys.stdin.buffer.read(),'pipeline_message_deposit.py','exec')"],
    ...
    def installed_syntax(self):
        self.run(["docker", "exec", "-e", "PYTHONDONTWRITEBYTECODE=1", "ai_engine",
                  "python", "-B", "-c",
                  "from pathlib import Path;compile(Path('/app/src/core/pipeline_message_deposit.py').read_bytes(),'pipeline_message_deposit.py','exec')"],
...
def check(native, root=LIVE_ROOT, candidate=CANDIDATE_ROOT):
    ...
    if fingerprint(root / TARGET) != before:
        raise Blocked("live_guard_changed_during_check")
...
def restore(native, backup, receipt, root=LIVE_ROOT):
    ...
    if current not in (receipt["before"], receipt.get("postimage")):
        raise Blocked("rollback_guard_conflict")
    ...
    if fingerprint(root / TARGET) == receipt.get("postimage"):
        ...
        if (stable(original_id) != stable(receipt["before"])
            ...
            raise Blocked("backup_guard_mismatch")
...
def apply(native, root=LIVE_ROOT, candidate=CANDIDATE_ROOT, base=BACKUP_BASE):
    ...
    backup = Path(tempfile.mkdtemp(prefix="prior-message-669b089-", dir=base)); os.chmod(backup, 0o700)
    ...
    atomic_bytes(backup_target, (root / TARGET).read_bytes(), checked["before"])
    atomic_bytes(backup / "agents-db-snapshot.sqlite", database,
                 {"uid": os.geteuid(), "gid": os.getegid(), "mode": "0600"})
    receipt = {"commit": COMMIT, "source_tree": SOURCE_TREE, "target": str(TARGET),
    ...
    try:
        ...
        def journal(temporary_id):
            if fingerprint(root / TARGET) != checked["before"]:
                raise Blocked("pre_replace_guard_changed")
        ...
        if stable(fingerprint(root / TARGET)) != AFTER:
            raise Blocked("postflight_guard_mismatch")
...
def rollback(native, backup, root=LIVE_ROOT):
    ...
    if (receipt.get("commit") != COMMIT or receipt.get("source_tree") != SOURCE_TREE
...
def main(argv=None):
    ...
    try:
        ...
        if args.apply or args.rollback:
            ...
            fd = os.open(BACKUP_BASE / ".runtime-prior-message-669b089.lock", os.O_CREAT | os.O_WRONLY | os.O_NOFOLLOW, 0o600)
