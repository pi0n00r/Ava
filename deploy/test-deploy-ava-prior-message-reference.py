...
"""Non-actuating tests for the Ava prior-message guard transaction."""
...
class FakeNative:
    ...
    def syntax(self, source): self.events.append("syntax"); compile(source, "guard.py", "exec")
...
class ActivationTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="ava-prior-message-activate-test-"); self.addCleanup(self.temp.cleanup)
        ...
        (self.candidate / "core").mkdir(parents=True)
        self.native = FakeNative()
        (self.root / deploy.TARGET).write_bytes(b"OLD = True\n")
        (self.candidate / deploy.TARGET).write_bytes(b"NEW = True\n")
        for path in (self.root / deploy.TARGET, self.candidate / deploy.TARGET): path.chmod(0o644)
        owner = {"uid": os.geteuid(), "gid": os.getegid()}
        self.before = deploy.fingerprint(self.root / deploy.TARGET)
        self.after = deploy.fingerprint(self.candidate / deploy.TARGET)
    ...
    def test_exact_single_guard_apply_and_rollback_never_restore_database(self):
        backup, receipt = self.apply()
        self.assertEqual(deploy.stable(deploy.fingerprint(self.root / deploy.TARGET)), deploy.AFTER)
        self.assertEqual(receipt["target"], "core/pipeline_message_deposit.py")
        self.assertEqual((backup / deploy.TARGET).read_bytes(), b"OLD = True\n")
        ...
        restored = deploy.fingerprint(self.root / deploy.TARGET)
    ...
    def test_apply_requires_two_fresh_zero_gates_before_one_engine_restart(self):
        self.apply()
        stop_index = self.native.events.index("stop")
        self.assertGreaterEqual(self.native.events[:stop_index].count("health"), 2)
        self.assertGreaterEqual(self.native.events[:stop_index].count("pbx_channels"), 2)
        self.assertGreaterEqual(self.native.events[:stop_index].count("snapshot"), 2)
        self.assertEqual(self.native.events.count("stop"), 1)
        self.assertEqual(self.native.events.count("start"), 1)

    def test_preimage_change_blocks_before_stop(self):
        (self.root / deploy.TARGET).write_bytes(b"FOREIGN = True\n")
        with self.assertRaisesRegex(deploy.Blocked, "live_guard_preimage_mismatch"): self.apply()
        self.assertNotIn("stop", self.native.events)

    def test_candidate_change_blocks_before_stop(self):
        (self.candidate / deploy.TARGET).write_bytes(b"FOREIGN = True\n")
        with self.assertRaisesRegex(deploy.Blocked, "candidate_guard_mismatch"): self.apply()
        self.assertNotIn("stop", self.native.events)
    ...
    def test_postflight_logical_change_rolls_back_guard_only(self):
        ...
        self.assertEqual(deploy.stable(deploy.fingerprint(self.root / deploy.TARGET)), deploy.stable(self.before))
    ...
    def test_external_postimage_change_prevents_rollback(self):
        backup, _ = self.apply(); (self.root / deploy.TARGET).write_bytes(b"THIRD = True\n")
        with self.assertRaisesRegex(deploy.Blocked, "rollback_guard_conflict"):
            deploy.rollback(self.native, backup, self.root)
...
class NativeContractTests(unittest.TestCase):
    ...
    def test_prepared_runtime_candidate_checks_owner_group_mode_and_size(self):
        ...
        case = ActivationTests("test_exact_single_guard_apply_and_rollback_never_restore_database")
        case.setUp()
        try:
